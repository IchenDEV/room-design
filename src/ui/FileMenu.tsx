...
export function FileMenu() {
  ...
  const runFileAction = async (action: () => Promise<string | null | void>) => {
    const err = await action();
    if (err) { toastErr(err); return; }
    const id = cloud ? activeProjectFile()?.cloudId : null;
    if (id) await restartCollab(id).catch((e) => toastErr(msg(e)));
  };
  const del = () => {
    if (confirm(`删除“${store.project.name}”？${cloud ? '云端与本地均会移除。' : '此操作不可撤销。'}`)) {
      runFileAction(() => deleteProjectFile(store, active?.id ?? '')).then(close);
    }
  };
  const saveVer = () => {
