...
export const bed: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  const frame = mat('#8a6a48', 0.75), mattress = mat('#f0ece2', 0.95);
  const blanket = mat(c, 0.92), pillow = mat('#ffffff', 0.95);
...
export const toilet: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  const por = mat(c, 0.25);
...
export const bathsink: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  g.add(box(w, h - 12, d - 4, mat(c, 0.7), 0, (h - 12) / 2 + 4, 0));
...
export const bathtub: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  const por = mat(c, 0.3);
