...
export const officedesk: Builder = (w, d, h, c, tex) => {
  ...
  g.add(box(mw, mw * 0.6, 2.5, mat('#14171c', 0.3), 0, h + mw * 0.3 + 9, -d * 0.22)); // 显示器
  g.add(cyl(2, 3, 8, leg, 0, h + 4, -d * 0.22));
  g.add(box(mw * 0.5, 1.6, 14, mat('#2e333a', 0.5), 0, h + 0.8, d * 0.05));           // 键盘
  return g;
};
...
export const officechair: Builder = (w, d, h, c, tex) => {
  ...
  g.add(box(w * 0.78, 7, d * 0.72, body, 0, h * 0.42, 0));
  g.add(box(w * 0.66, h * 0.42, 6, body, 0, h * 0.62, -d * 0.3));
  for (const s of [-1, 1]) g.add(box(4, 3, d * 0.4, metal, s * w * 0.36, h * 0.5, 0));
  return g;
};
...
export const filecabinet: Builder = (w, d, h, c, tex) => {
  ...
  for (let i = 0; i < n; i++) {
    const y = (h / n) * (i + 0.5);
    g.add(box(w - 6, h / n - 5, 2, front, 0, y, d / 2 + 1));
    g.add(box(w * 0.3, 2.4, 2.4, mat('#3f444a', 0.4, 0.6), 0, y + h / n / 2 - 7, d / 2 + 2.4));
  }
  return g;
};
