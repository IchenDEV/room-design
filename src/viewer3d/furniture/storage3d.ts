...
export const wardrobe: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  const body = mat(c, 0.8), front = mat(shade(c, 14), 0.75), metal = mat('#c9c3b8', 0.3, 0.8);
...
export const nightstand: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  g.add(box(w, h - 8, d, mat(c, 0.78), 0, (h - 8) / 2 + 8, 0));
  g.add(box(w - 6, (h - 8) / 2 - 4, 2, mat(shade(c, 16), 0.75), 0, h * 0.62, d / 2 + 1));
  g.add(cyl(1.4, 1.4, 1.4, mat('#cfc9bd', 0.3, 0.8), 0, h * 0.62, d / 2 + 2.4));
  g.add(box(w * 0.8, 8, d * 0.8, mat('#d8cfc0', 0.85), 0, h + 4, 0)); // 台灯底座装饰
  return g;
};

export const dresser: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  g.add(box(w, h - 4, d, mat(c, 0.78), 0, (h - 4) / 2, 0));
  g.add(box(w, 4, d + 3, mat(shade(c, 20), 0.7), 0, h - 2, 0));
...
export const fridge: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  const body = mat(c, 0.35, 0.45), seam = mat(shade(c, -35), 0.4, 0.4);
  g.add(box(w, h, d, body, 0, h / 2, 0));
...
export const washer: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  g.add(box(w, h, d, mat(c, 0.4, 0.35), 0, h / 2, 0));
...
export const counter: Builder = (w, d, h, c) => {
  const g = new THREE.Group();
  const body = mat(c, 0.8), top = mat('#e6e1d5', 0.5), dark = mat(shade(c, -30), 0.8);
