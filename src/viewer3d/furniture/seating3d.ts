...
export const sofa: Builder = (w, d, h, c, tex) => {
  ...
  const fab = mat(c, 0.92, 0, tex), cush = mat(shade(c, 18), 0.95, 0, tex), wood = mat('#6f553c', 0.7);
...
export const stool: Builder = (w, d, h, c, tex) => {
  const g = new THREE.Group();
  g.add(box(w, 6, d, mat(c, 0.85, 0, tex), 0, h - 3, 0));
...
export const bench: Builder = (w, d, h, c, tex) => {
  const g = new THREE.Group();
  g.add(box(w, 6.5, d, mat(c, 0.8, 0, tex), 0, h - 3.2, 0));
